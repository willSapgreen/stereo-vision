...
public:
    ...
    FrameCaptureThread(StereoImage *stereo_image, CalibIOKITTI *calib, bool cam_left,
                       QMutex *capture_mutex, QObject *parent = 0);
    ~FrameCaptureThread();
...
private:
    ...
    int                  cam_ind;
    CalibIOKITTI         *calib;
    bool                 cam_left; // true if the left camera is the reference camera.
    int                  shutter;
    bool                 recording;
